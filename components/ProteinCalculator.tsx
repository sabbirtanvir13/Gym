...
export default function ProteinCalculator() {
  ...
  const handleCalculate = (e: React.FormEvent) => {
    ...
    setErrors({});
    const calculatedResult = calculateDailyProtein(input);
  ...
  return (
    <section id="protein-calculator" className="relative py-24 lg:py-32">
      {/* Background glow decorations matching site theme */}
      ...
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <SectionHeading
          eyebrow="Calculate Your Daily Protein"
          title="Personalized Protein Target"
          subtitle="Fuel your muscle growth, speed up recovery, and hit your fitness goals with a custom daily protein strategy calculated for your body."
          center
        />
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-12 lg:items-start">
          ...
          <motion.div
            ...
            className={`rounded-3xl border border-white/10 bg-charcoal/80 p-6 backdrop-blur-xl transition-all lg:p-8 ${
              hasCalculated && result ? "lg:col-span-7" : "lg:col-span-12 max-w-4xl mx-auto"
            }`}
          >
          ...
          <AnimatePresence mode="wait">
            {hasCalculated && result && (
              <motion.div
                ...
                className="lg:col-span-5 rounded-3xl border border-accent/30 bg-gradient-to-b from-charcoal/90 via-charcoal/80 to-ink-2/90 p-6 backdrop-blur-xl shadow-[0_0_40px_rgba(0,0,0,0.5),0_0_25px_var(--theme-accent-glow)] lg:p-8"
              >
                {/* Result Header Badge */}
                <div className="flex items-center justify-between border-b border-white/10 pb-5">
                  <div className="flex items-center gap-2">
                    <Sparkles className="text-accent animate-spin-slow" size={20} />
                    <span className="text-xs font-bold uppercase tracking-widest text-accent">
                      YOUR DAILY PROTEIN TARGET
                    </span>
                  </div>
                  <span className="rounded-full bg-accent/15 px-3 py-1 text-[11px] font-extrabold text-accent border border-accent/20">
                    ESTIMATE
                  </span>
                </div>
                ...
                <div className="py-6 text-center">
                  <div className="font-display text-5xl font-extrabold text-white tracking-tight sm:text-6xl">
                    <span className="text-gradient drop-shadow-sm">{result.recommendedTarget}</span>
                    <span className="text-2xl font-bold text-accent ml-1.5">g/day</span>
                  </div>
                  <p className="mt-2 text-sm text-ash font-medium">
                    Recommended Range:{" "}
                    <strong className="text-white font-semibold">
                      {result.minRange}–{result.maxRange} g/day
                    </strong>
                  </p>
                </div>

                {/* Animated Range / Progress Bar */}
                <div className="mb-6 rounded-2xl border border-white/5 bg-ink/60 p-4">
                  <div className="flex items-center justify-between text-xs font-bold text-ash mb-2">
                    <span>Daily Target Scale</span>
                    <span className="text-accent font-extrabold">{result.recommendedTarget}g</span>
                  </div>

                  {/* Progress track */}
                  <div className="relative h-3 w-full overflow-hidden rounded-full bg-white/10 p-0.5">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${result.percentagePosition}%` }}
                      transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
                      className="h-full rounded-full bg-gradient-to-r from-accent to-secondary shadow-[0_0_12px_var(--theme-accent-glow)]"
                    />
                  </div>

                  <div className="mt-2 flex justify-between text-[11px] font-semibold text-ash-2">
                    <span>{result.minRange}g (Min)</span>
                    <span>{result.maxRange}g (Max)</span>
                  </div>
                </div>

                {/* Meal Breakdown Grid */}
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <div className="rounded-xl border border-white/5 bg-ink-2/80 p-3.5 text-center">
                    <span className="block text-[11px] font-bold uppercase tracking-wider text-ash">
                      Protein per meal
                    </span>
                    <span className="mt-1 flex items-center justify-center gap-1 text-lg font-extrabold text-white">
                      <Flame size={16} className="text-accent" />
                      Approx. {result.proteinPerMeal} g
                    </span>
                  </div>
                  ...
                  <div className="rounded-xl border border-white/5 bg-ink-2/80 p-3.5 text-center">
                    ...
                    <span className="mt-1 flex items-center justify-center gap-1 text-lg font-extrabold text-white">
                      ...
                      {result.suggestedMeals} meals/day
                    </span>
                  </div>
                </div>

                {/* Explanation and Disclaimer */}
                <div className="space-y-3 rounded-xl border border-white/5 bg-ink/40 p-4 text-xs text-ash leading-relaxed">
                  <p className="flex items-start gap-2">
                    <Info size={14} className="mt-0.5 shrink-0 text-accent" />
                    <span>
                      Based on your body weight, activity level, and fitness goal, this is an
                      estimated daily protein target.
                    </span>
                  </p>
                  <p className="pt-2 border-t border-white/5 text-[11px] text-ash-2 italic">
                    * This is a general estimate and is not medical or nutritional advice.
                  </p>
                </div>
              </motion.div>
            )}
